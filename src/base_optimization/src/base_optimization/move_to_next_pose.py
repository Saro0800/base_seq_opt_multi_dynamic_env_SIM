...
# polling rate of move_base state and check for obstacles
...
def request_reoptimization(cancelled_target):
    ...
    if reopt_success and len(collected_poses) > 0:
        rospy.loginfo("Re-optimization succeeded. Inserting %d new base poses "
        ...
        rospy.logwarn("Re-optimization failed or timed out. "
...
# instantiate the dynamic obstacle monitor
...
# start checking if a new base pose arrived every 2 seconds
while not rospy.is_shutdown():
    ...
    if len(opt_base_poses) <= 0:
        ...
        # send goal base and poll for result while checking for dynamic obstacles
        goal_reached = False
        ...
        while not rospy.is_shutdown() and not goal_reached and not goal_cancelled:
            ...
            while not rospy.is_shutdown():
                ...
                if state in (actionlib.GoalStatus.ACTIVE,
                    ...
                    try:
                        tf_stamped = tf_buffer.lookup_transform("octomap_frame",
                                                                "locobot/base_footprint",
                                                                rospy.Time(0),
                        ...
                        if obstacle_monitor.is_target_obstructed(
                            ...
                            rospy.logwarn("Target appears obstructed. Waiting 5 s to confirm...")
                            rospy.sleep(5.0)
                            ...
                            if obstacle_monitor.is_target_obstructed(
                                ...
                                if obstacle_monitor.is_obstacle_mobile(nxt_base_pose.base_pose.pose):
                                    rospy.logwarn("Dynamic obstacle confirmed at target (within frustum)! "
                                                  "It is a mobile agent. Cancelling and re-queuing target at end.")
                                    # put the same target back at the end of the queue
                                    opt_base_poses.append(nxt_base_pose)
                                    viz_all_base_poses()
                                else:
                                    rospy.logwarn("Dynamic obstacle confirmed at target (within frustum)! "
                                                  "It is a static obstacle. Requesting re-optimization...")
                                    # request re-optimization for the gripper poses
                                    # that this base pose was supposed to reach
